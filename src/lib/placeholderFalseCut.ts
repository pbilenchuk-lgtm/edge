...
// 17.2% ложных срезов. Из этого верного наблюдения были выведены ДВЕ неверные правки (корроборация
// манилайном), обе замерены ретро и откачены: признак не разделял брак (6/33 против 4/25), а цена
// составила 21 верный срез из 48. Правило вернулось к исходному; 17.2% — принятая цена, а не недосмотр.
//
...
/**
 * Пути, ПРОИЗВОДИМЫЕ правилом сегодня, и пути СНЯТЫЕ.
 *
 * 08.08 через этот список прошли две правки и откат: `no_ask` уходил в снятые, потом вернулся, а
 * `no_book_ml`/`no_ask_ml` прожили меньше суток. Снятые НЕ удаляются: их строки — единственное основание
 * решений, и без них через месяц «правило поменяли» стало бы неотличимо от «правило всегда было таким».
 * История append-only и здесь.
 *
 * Разделение живых и снятых несёт нагрузку в трёх местах, и все три пришлось чинить по отдельности:
 * `totals`, `verdict` и `worst` считаются ТОЛЬКО по действующим — иначе снятое правило обвиняет живое.
 */
