...
// ── ПРАВИЛО КЛАССА: p-VALUE ОБЯЗАНО НАЗЫВАТЬ СВОЮ ЕДИНИЦУ ──────────────────
// [ратифицировано владельцем 02.08.2026 — дешёвая прививка от четвёртого раза]
//
// «Единица измерения» ударила ТРИЖДЫ: вердиктные ячейки, refusal-shadow и — 02.08 — чуть не основала
// прод-механизм. Каждый раз механика одна: строки одного матч×рынка считались независимыми испытаниями.
// p=0.00004 на «24 из 24» превратилось в p=0.28, как только 24 строки схлопнулись в ТРИ сигнала.
//
// Правило: ЛЮБОЕ p-value, произнесённое в этом проекте — в отчёте, в логе, в чате, — обязано называть
// единицу, на которой посчитано. Не «p=0.16», а «p=0.16 на 9 сигналах». Читатель тогда видит знаменатель
// раньше, чем вывод, и подмена единицы перестаёт быть невидимой.
//
// Это не форматирование ради красоты: число без единицы НЕ ЯВЛЯЕТСЯ утверждением, потому что его нельзя
// проверить. Хелпер существует затем, чтобы правило соблюдалось дешевле, чем нарушалось.
export type PUnit = "сигналах" | "записях" | "матчах" | "ставках" | "днях";
export function pWithUnit(p: number | null, n: number, unit: PUnit = "сигналах"): string {
  if (p == null) return `p не посчитан (n=${n} ${unit})`;
  return `p=${p < 0.0001 ? p.toExponential(1) : p.toFixed(p < 0.01 ? 4 : 3)} на ${n} ${unit}`;
}
