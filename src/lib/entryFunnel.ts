...
// путь пишет причину, которой словарь не знает, то есть словарь отстал от кода.
//
// ОПРЕДЕЛЕНИЕ МЕТОДА [ратифицировано владельцем 02.08.2026] — АСИММЕТРИЧНАЯ ГАРАНТИЯ ВМЕСТО СИММЕТРИЧНОЙ
// ЛЖИ: «ноль невязки не доказывает полноту, но ненулевая доказывает пробел».
//
// Это правило шире воронки и применяется всюду, где сторож считает по неполному источнику. Симметричное
// утверждение («сходится ⇒ всё учтено») требует доказательства полноты источника, которого у нас нет ни
// для прозы, ни для журнала, ни для пульса. Асимметричное требует только корректности ОДНОГО направления
// — и потому его можно держать честно. Практическое следствие: сторож обязан быть построен так, чтобы его
// МОЛЧАНИЕ ничего не утверждало, а его КРИК был доказательством. Всё, что нарушает этот порядок, —
// ложная тревога либо немой ноль; за сутки 02.08 наблюдаемость поймала по три экземпляра каждого.
//
