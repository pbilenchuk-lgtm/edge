...
//   e11 [07.08] ЦЕНА, ПРОТИВ КОТОРОЙ СЧИТАЕТСЯ КРАЙ, СМЕНИЛАСЬ — значит сменилось МНОЖЕСТВО СИГНАЛОВ.
//       #120: стратег и очередь кандидатов считают эдж от ИСПОЛНИМОГО АСКА, а не от мида. Замер 151
//       матч-лога: медиана разрыва аск−котировка +27.5¢ (макс +55.5¢), и на аске край уходил в минус
//       (−7 / −14 / −18 п.п.). То есть часть прежних «входов» перестаёт существовать — осознанное
//       ужесточение, а не улучшение.
//       #121: рынок с ценой ровно на 50¢ при неразмеченной книге или при перекошенном манилайне скрыт от
//       стратега ДО вызова (1737 из 2487 теннисных рынков стоят там; временной карантин ловил 70).
//       Оба меняют ВХОД, а не учёт: сигнал, существовавший до отметки, после неё может не существовать —
//       следовательно, до и после это разные когорты, и вердикт читается внутри эпохи, а не поперёк.
//
//       СОСТАВ ЭПОХИ, ЗАПИСАННЫЙ ЯВНО: **e11 = #120 + #121 + ft_blind-карантин-фикс.**
//       Третьим изменением в ту же границу уехал фикс `not_score_graded` (#119/#141): когорта ft_blind
//       выходила из `settle_suspect/uncovered_state`, где её держало правило «нет счёта» — карантин,
//       определённый ровно тем признаком, которым определена сама когорта. Деньги фикс НЕ трогает.
//       ЗАЧЕМ ЭТА СТРОКА. Граница несёт ТРИ изменения поведения, а атрибуция сдвига по умолчанию
//       достаётся эпохе. Если когорта ft_blind поведёт себя иначе после e11, читатель обязан знать, что
//       у отметки стоял и её собственный фикс, — иначе он припишет сдвиг ценовым изменениям #120/#121 и
//       получит согласие двух гипотез там, где их не проверяли по отдельности. Бутерброд был исключением
//       по необходимости ветки (фикс уже ратифицирован и денежно нейтрален), а не приёмом: следующая
//       эпохальная отметка едет одна.
export const CODE_VERSION = "e11";
