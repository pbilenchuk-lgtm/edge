...
export interface OverreactionGate {
  ...
  excluded: { void: number; cashOut: number; preEpoch: number; crossEpoch: number };
...
export function buildOverreactionGate(db: Database, target = 30, cleanEpochMin = 5): OverreactionGate {
  const rows = db.prepare(
    `SELECT b.status, b.settled_by, b.code_version, b.exit_code_version
  ...
  ).all() as { status: string; settled_by: string | null; code_version: string | null; exit_code_version: string | null }[];

  let cleanCycles = 0, won = 0, lost = 0;
  const excluded = { void: 0, cashOut: 0, preEpoch: 0, crossEpoch: 0 };
  ...
  const note = verdict === "gate_open"
    ...
    : `⏳ КОПИМ: ${cleanCycles}/${target} чистых циклов (won ${won} / lost ${lost}) — до n≥30 вердикт по Overreaction ещё преждевременный; исключено ${excluded.void} void, ${excluded.cashOut} кэш-аут, ${excluded.preEpoch} до-e5, ${excluded.crossEpoch} cross-epoch`;
