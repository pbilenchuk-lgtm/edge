...
export function seedDatabase(db: Database): void {
  ...
  // phases: [bets, wins, pnl, clv] for "До матча" then "В течение матча"
  //         (post-event folded into live). mgmt: [actualPnl, heldToEndPnl, managed] | null.
  type PhaseTuple = [number, number, number, number | null];
  const quality: Record<string, {
    brier: number; clv: number; samples: number; calib: Array<[string, number, number]>;
    pre: PhaseTuple; live: PhaseTuple; mgmt: [number, number, number] | null; equity: number[];
  }> = {
    edge: { brier: 0.182, clv: 3.4, samples: 24, calib: [["50-60%", 55, 53], ["60-70%", 65, 67], ["70-80%", 75, 72], ["80%+", 85, 88]],
      pre: [24, 13, 42.5, 1.8], live: [18, 14, 184.5, 4.7], mgmt: [227.0, 158.0, 14], equity: [0, 10, -5, 40, 120, 90, 160, 145, 210, 227] },
    flat: { brier: 0.213, clv: 0.8, samples: 24, calib: [["50-60%", 55, 51], ["60-70%", 65, 64], ["70-80%", 75, 70], ["80%+", 85, 79]],
      pre: [24, 14, 41.0, 0.8], live: [0, 0, 0, null], mgmt: null, equity: [0, 2, -2, 5, 10, 8, 15, 20, 35, 41] },
    kelly: { brier: 0.195, clv: 2.1, samples: 18, calib: [["50-60%", 55, 56], ["60-70%", 65, 63], ["70-80%", 75, 74], ["80%+", 85, 83]],
      pre: [18, 11, 58.0, 1.5], live: [8, 5, 34.5, 2.9], mgmt: [92.5, 101.0, 6], equity: [0, 10, 5, 20, 15, 30, 45, 40, 55, 92] },
    tn1: { brier: 0.24, clv: -0.5, samples: 6, calib: [["50-60%", 55, 52], ["60-70%", 65, 61]],
      pre: [6, 3, -4.0, -0.5], live: [2, 1, 6.0, 0.4], mgmt: null, equity: [0, -2, 2, -4, 5, 2] },
  };
  const phaseRow = (id: "pre" | "live", label: string, [bets, wins, pnl, clv]: PhaseTuple) => ({ id, label, bets, wins, pnl, clv });
  for (const [sid, q] of Object.entries(quality))
    R.upsertQuality(db, {
      ...
      phases: [phaseRow("pre", "До матча", q.pre), phaseRow("live", "В течение матча", q.live)],
      mgmt: q.mgmt ? { actualPnl: q.mgmt[0], heldToEndPnl: q.mgmt[1], managed: q.mgmt[2] } : null,
      equity: q.equity,
      updated_at: T,
    });
}
...
// ============================================================
const PROMPT_MATCH_ANALYTICS = `Ты — профессиональный футбольный аналитик. Твоя задача — собрать и структурировать
ВСЕ значимые данные по конкретному матчу, чтобы на их основе строилась стратегия ставок.
Ты НЕ даёшь ставок и НЕ оцениваешь edge — только факты и их интерпретацию.

ОБЯЗАТЕЛЬНЫЙ ПОРЯДОК:

1. СОСТАВЫ — используй подтверждённые стартовые XI (в контексте ниже даны реальные составы и события с ESPN, если матч начался/составы объявлены). Если официальных ещё нет — прямо скажи это. Выпиши обе XI со схемой (формацией) и ключевыми заменами на лавке (джокеры).

2. СЮРПРИЗЫ СОСТАВА — отдельно выдели неожиданное: звезда/бомбардир на лавке или травмирован; смена схемы (напр. 3 защитника вместо 4); ключевой защитник/вратарь отсутствует → дыра в обороне; игрок не на своей позиции.

3. ТАКТИКА И СТРАТЕГИЯ КАЖДОЙ КОМАНДЫ: как каждая планирует играть (контроль/автобус/контратаки/прессинг); где сильные и УЯЗВИМЫЕ зоны; кто главная угроза и как её нейтрализуют.

4. МОТИВАЦИЯ И КОНТЕКСТ: что каждой нужно от матча (победа / хватает ничьи / овертайм в запасе); влияет ли стадия турнира на осторожность/результативность; усталость/ротация.

5. ОЦЕНКА ПО ЛИНИЯМ (поимённо, кратко): Атака А vs Оборона Б; Атака Б vs Оборона А; вратари; кто реально может ЗАБИТЬ, а кто беззуб.

6. ФОРМА И СТАТИСТИКА: последние результаты, забитые/пропущенные, xG; H2H. ВАЖНО: различай «не реализует / расточителен» от «беззуб» — команда с большим ОБЪЁМОМ моментов забьёт.

7. ИТОГ — определи ТИП матча: Тип А «Фаворит против автобуса» (голы поздно, после 55'); Тип Б «Открытый» (обе атакуют, голы с начала); Тип В «Тесный равный» (осторожные, возможна ничья → овертайм). Одним абзацем: какой профиль исхода наиболее вероятен и почему.

ФОРМАТ: структурированно, по разделам, без воды. Факты и интерпретация, без рекомендаций по ставкам (это следующий этап). В конце дай вероятности ключевых рынков из списка котировок.`;
