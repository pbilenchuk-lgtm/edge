...
БАЗОВЫЙ ПРИНЦИП: Предматчевое ПРЕДСКАЗАНИЕ исхода — НЕ доказанный edge (рынок эффективен). Реальный edge — УПРАВЛЕНИЕ внутри матча. Предматчевые ставки — малые/умеренные. Крупный размер только на внутриматчевый edge высокой уверенности.

── А. ПОРОГИ (соблюдать всегда) ──
РАЗМЕР ПО EDGE (edge = моя P минус implied-вероятность цены): edge ≥ 10% → максимум (1.0); 7–10% → 0.75; 5–7% → половина (0.5); 3–5% → четверть (0.25); < 3% → НОЛЬ (пропуск).
ПОТОЛКИ: макс. экспозиция на матч 1.5 единицы; макс. 2 ставки-направления на матч (в одну сторону, усиливающие); макс. 4 матча за сессию; стоп-лосс сессии −20% банка → СТОП.
ЛИКВИДНОСТЬ: линия > $1M → максимально эффективна, edge дисконтировать вдвое; не входить при edge < 5%.
...
export function seedMinimal(db: Database): void {
  ...
  R.insertStrategy(db, {
    ...
    params: { minEdge: 3, tiers: [[10, 0.16], [7, 0.12], [5, 0.08], [3, 0.04]], maxPerBet: 0.16, stop: -0.20, takeProfit: 0.6, exitStop: 0.5 },
  });
}
