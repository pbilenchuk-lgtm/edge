...
**Контекст как отдельные драйверы.** Мотивация, усталость/календарь, перелёты, травмы ключевых, погода, судья — каждый как отдельный driver с направлением и величиной.

## OVERRIDES — когда вмешиваться в Пуассон

Пуассон механически разворачивает xG в рынки, но слеп к тактике и турнирной мотивации. Выпускай override ТОЛЬКО когда у тебя есть конкретное футбольное знание, которого нет в xG. Примеры: обоим достаточно ничьей → распределение голов сожмётся, Over переоценён → override вниз по тоталам; команда обязана отыгрывать 2 мяча → раскроется, тотал выше механического. Каждый override ОБЯЗАН иметь конкретный reason. Без причины — не выпускай.

## ОПРЕДЕЛЕНИЕ match_type

Определи по доступным рынкам во входных данных: есть трёхисходный рынок с ничьёй → group. Есть «Team to Advance»/«Extra Time» и НЕТ «Draw» → knockout (P(extra_time) ≈ P(ничья в 90)). Не ясно → uncertain, запиши в unknowns, НЕ гадай. Всегда пиши match_type_reason.
...
Команды, тип турнира/стадия, доступные рынки (для match_type), контекст (составы если вышли — иначе ожидаемые, форма в underlying-метриках, травмы, календарь, место, погода). Если составы НЕ подтверждены — работай на ожидаемых и явно пропиши в unknowns, что не подтверждено и как это двигает оценку.
...
## КАК ОФОРМЛЯТЬ
Поправки к core (множитель/сдвиг к xG/тайм-долям, напр. «Мехико, высота → −8% к xG во 2-м тайме»); новый driver турнирной мотивации; новый scenario (напр. «параллельный матч меняет мотивацию»); поправка к calibration (обычно вниз по confidence — несыгранность/малая выборка); поправка к overrides (мотивация → сжатие/раскрытие тоталов).

Мало специфики — выдавай короткий список или пустые массивы. Не выдумывай ради объёма. Лучше две обоснованные поправки, чем десять натянутых.`;
