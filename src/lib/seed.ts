...
const PROMPT_WC_CONTEXT = `КОНТЕКСТ ТУРНИРА — ЧМ-2026 (калибруй ожидания фактами, а не ощущениями):

1. ФОРМАТ ПЛЕЙ-ОФФ: Раунд 32 (48 команд): ничья в осн. время → 30 мин овертайма → пенальти. Тоталы O/U обычно считаются за 90 мин (признак: есть отдельные рынки "Extra Time Yes/No" и "Penalty Shootout"). Пенальти в тоталы НЕ входят. Следствие: аутсайдеру часто выгодна ничья → пенальти → он играет на 0:0.

2. ОПИСАТЕЛЬНАЯ СТАТИСТИКА (только для калибровки допущений, НЕ доказательство edge, look-ahead bias): среднее ~3.0 гола/матч, Over 2.5 заходит ~56%; доля поздних голов (после 55') высока (~53% в матчах типа А); мотивированный класс-фаворит чаще ДОЖИМАЕТ, чем вязнет.

3. КОНТЕКСТ СТАДИИ: групповой этап vs плей-офф; кому нужны голы (разница мячей, борьба за выход) — влияет на открытость.

4. ПАТТЕРНЫ (применять ТОЛЬКО при заданных условиях): «Фаворит дожимает автобус поздно (после 55')»; «Ничья в осн. время для ИСТИННО равных → овертайм»; «Камбэк фаворита на дне после раннего гола аутсайдера»; «Класс-фаворит дожимает в осн. время».

5. АНТИ-ПАТТЕРНЫ: не штамповать Under (среднее ~3.0, Over ~56%); не ставить «не забьёт» против команды с объёмом моментов; не переоценивать осторожность класс-фаворита; мега-ликвидные линии ($1M+) максимально эффективны — edge иллюзорен.`;
