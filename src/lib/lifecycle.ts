...
export function footballZombieMap(
  ...
  for (const mk of markets) {
    ...
    const zi = { label: mk.label, priceCents: mk.price, askCents: mk.ask_cents ?? null, gsProb: gs?.prob ?? null, groupSpreadCents: spreads.get(mk.label) ?? null, bookAgeMin: bookAge, live: true, matchFinished: false };  // карта строится только для state==="live" — матч по определению не завершён
