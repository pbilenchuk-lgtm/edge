...
function MatchCard({ match, catalog, comp, compBudget, shares, shareRows, riskProfiles, onRefreshOdds, onReassess, onAnalyze, onResumeAnalyze, oddsErrKey }: any) {
  ...
  // Tennis has NO squad/lineup and is scout-authoritative: it flips to live only when the
  // scout sees in-play, NOT at the scheduled time. So the football "СОСТАВ" badge is wrong —
  // pre-kickoff reads «СКОРО», and a match already past its scheduled start but not yet live
...
const FILE_GROUPS: { title: string; items: { label: string; url: string; file: string; hint?: string }[] }[] = [
  { title: "Стратегии × профили риска", items: [
    ...
    { label: "Отказы стратега: дисциплина или перекрут (R5)", url: "/api/profiles?report=refusal_shadow", file: "refusal-shadow.json", hint: "Батч-10: стратег отказал в 22 из 28 матчей. Каждый тотал с краем ≥5%, от которого он ушёл, записан would-be и резолвится тем же кодом, что и деньги. Вердикт только с n≥25; до созревания жёсткость НЕ трогаем." },
    { label: "Своевременность предматча + TAM ft_blind (R3)", url: "/api/profiles?report=prematch_timeliness", file: "prematch-timeliness.json", hint: "Доля предложений ДО кикоффа (цель ≥90%): опоздание стамплит origin=live, а ft_blind отвергает live-тезис на слепой фикстуре. Плюс честный TAM режима — плейсхолдеры не инвентарь." },
    { label: "Согласованность ног одного рынка (Z2а)", url: "/api/profiles?report=leg_consistency", file: "leg-consistency.json", hint: "Один рынок одного матча — ОДИН контракт. suspect = додержанные ноги разошлись в разные стороны (двойной сеттл/ошибка разметки, искажает win-rate); partial_explained = частичный срез + додержанный остаток (законно, но сигнал схлопывается в void по M6)." },
    { label: "Защитные срезы: продали дно или смерть тезиса?", url: "/api/profiles?report=stop_counterfactual", file: "stop-counterfactual.json", hint: "P3 (батч-9): по каждому защитному выходу — лучшая цена того же рынка в следующие 30м против цены среза. Критерий зафиксирован ДО данных: медиана недобора ≥5¢ И ≥20% при n≥30 → срезы шумовые, руль на состояние тезиса. Отдельный флаг: доля случаев, где hard-stop переехал тезисную подавку (≥10% — приоритетный баг сам по себе)." },
