...
import { ANALYZE_PRE_HOURS } from "../src/lib/lifecycle.js";
...
for (const c of R.listCompetitions(db, "football")) {
  ...
  for (const m of R.listMatches(db, c.id)) {
    ...
    // Football is analyzed ONLY once the real XI is out (без состава не торгуем → не анализируем).
    ...
    else if (awaiting) auto = "ждёт состав (ESPN)";
    else if (h > ANALYZE_PRE_HOURS) auto = `ждёт (>${ANALYZE_PRE_HOURS}ч)`;
    else auto = "запустится (состав есть)";
...
console.log(`\n${total} матч(ей) в ближайшие ${HORIZON_H}ч. Футбол анализируется только когда вышел реальный состав (ESPN).`);
