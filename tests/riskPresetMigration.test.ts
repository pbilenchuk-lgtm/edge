...
test("(б) эпоха разрезает когорты: ставка до отметки и после — не одна выборка", () => {
  ...
  migrateRiskPresetsToCode(db, NOW);
  const cut = JSON.parse(R.metaGet(db, PRESET_EPOCH_CUT_KEY)!);
  // ИНВАРИАНТ: отметка разреза записывает ту эпоху, при которой миграция реально шла. Он и держится.
  assert.equal(cut.codeEpochAtCut, CODE_VERSION);
  // Прежде здесь стоял литерал "e10" — он фиксировал, что эпоха поднята ТЕМ ЖЕ деплоем, что и синхронизация
  // пресетов (02.08). Как утверждение о ТЕКУЩЕЙ константе он живёт ровно до следующей эпохи и ломается на
  // ней, ничего не защитив: на проде миграция маркер-защищена и её отметка навсегда осталась "e10", а в
  // свежей базе она законно запишет сегодняшнюю. Поэтому проверяется то, что действительно нельзя нарушить:
  // эпоха монотонна и на момент разреза conservative-2.0 была не ниже e10.
  assert.ok(epochNum(CODE_VERSION) >= 10, `эпоха не откатывается назад (сейчас ${CODE_VERSION})`);
