...
// [ОТКАТ 08.08] Две правки требовали корроборации; ретро опровергло обе (см. placeholderStructural.ts).
// Поведение исходное: молчание книги режет само, 17.2% ложных — измеренная и принятая цена.
test("нет аска, спред известен — режем и без манилайна", () => {
  const r = structuralPlaceholders([mk(ML, 48), mk(PROP, 50, { ask: null })]);
  assert.equal(r.length, 1);
  assert.equal(r[0]!.path, "no_ask");
});
...
test("книга молчит по всем полям — свой путь в учёте, поведение то же", () => {
